...
async function handlePlayerMove(ws, data) {
  ...
  try {
    ...
    const result = calculateMoveResult(ws.pokemon, ws.opponent.pokemon, data.move);
    
    // Mensaje para el jugador que hizo el movimiento
    ...
    // Verificar si el movimiento terminó la batalla
    if (result.battleEnded) {
      await endBattle(ws.roomId, result.winner);
      
      // Enviar mensaje de fin de batalla a ambos jugadores
      const endMessage = JSON.stringify({
        type: "battleEnd",
        winner: result.winner,
        message: `¡${result.winner} ha ganado la batalla!`
      });
...
function calculateMoveResult(attackerPokemon, defenderPokemon, moveName) {
  ...
  const defenseStat = move.type === "physical" ? defenderPokemon.stats.defense : defenderPokemon.stats.specialDefense;
  
  // Fórmula de daño básica: (2 * nivel / 5 + 2) * poder * (ataque / defensa) / 50 + 2
  ...
  const battleEnded = finalDamage >= defenderPokemon.stats.hp;
  let winner = null;
  
  if (battleEnded) {
    winner = attackerPokemon.name; // El atacante gana si el defensor se queda sin HP
  }
